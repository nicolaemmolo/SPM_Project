...
//
// compile:
// mpicxx -std=c++20 -O3 -I -Iinclude -Iinclude/fastflow-master UTWavefrontMPI_FF.cpp -o UTWMPIFF
//
...
/* Wavefront (parallel version with static scheduling using FastFlow and MPI)
 * @param M: matrix
 * @param N: size of the matrix
 ...
 */
void wavefront_parallel_static_ff_mpi(std::vector<std::vector<double>> &M, const uint64_t &N, const uint32_t &T, const int &rank, const int &size) {
    ParallelFor pf(T);
...
/* Wavefront (parallel version with dynamic scheduling using FastFlow and MPI)
 * @param M: matrix
 * @param N: size of the matrix
 * @param T: number of threads
 * @param rank: MPI rank
 * @param size: number of MPI processes
 */
void wavefront_parallel_dynamic_ff_mpi(std::vector<std::vector<double>> &M, const uint64_t &N, const uint32_t &T, const int &rank, const int &size) {
    ParallelFor pf(T);
...
int main(int argc, char *argv[]) {
    ...
    if (mode == "ps") {
        ...
        wavefront_parallel_static_ff_mpi(M, N, nodes, rank, size);
    ...
    if (mode == "pd") {
        ...
        MPI_Barrier(MPI_COMM_WORLD); // Synchronize processes before timing
        double start_time = MPI_Wtime();
        wavefront_parallel_dynamic_ff_mpi(M, N, nodes, rank, size);
        double end_time = MPI_Wtime();
        execution_time = end_time - start_time;
    }
