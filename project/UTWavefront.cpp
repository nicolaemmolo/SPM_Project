...
// Modified work function to compute the wavefront element
void compute_element(std::vector<std::vector<double>> &M, const uint64_t &m, const uint64_t &k) {
...
void wavefront_sequential(const std::vector<double> &M, const uint64_t &N) {
	for(uint64_t k=1; k<N; ++k) { // for each upper diagonal
		for(uint64_t i=0; i<(N-k); ++i) { // for each element in the diagonal
			//calculate_diagonal_element(M,m,k)
		}
	}
}
...
void wavefront_parallel_static(const std::vector<double> &M, const uint64_t &N, const uint32_t &T) {
...
void wavefront_parallel_dynamic(const std::vector<double> &M, const uint64_t &N, const uint32_t &T) {
...
int main(int argc, char *argv[]) {
	...
	auto init=[&]() {
		for (uint64_t m=0; m<N; ++m) {
			M[m * N + m] = static_cast<double>(m+1) / static_cast<double>(N);
		}
	};
