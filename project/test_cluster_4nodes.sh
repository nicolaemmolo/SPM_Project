...
mpi_execution() {
    for N in $($DIMENSIONS_LIST); do
        for rep in $(seq 1 $REPETITIONS); do
            mpirun ./UTWMPI $N results_UTWMPI_4.csv $nodes
        done
    done
}
...
mpi_omp_static_execution() {
    for N in $($DIMENSIONS_LIST); do
        for T in $($THREADS_LIST); do
            for rep in $(seq 1 $REPETITIONS); do
                mpirun ./UTWMPI_OMP $N $T ps results_UTWMPI_OMP_static_4.csv $nodes
            done
        done
    done
}
...
mpi_omp_dynamic_execution() {
    for N in $($DIMENSIONS_LIST); do
        for T in $($THREADS_LIST); do
            for rep in $(seq 1 $REPETITIONS); do
                mpirun ./UTWMPI_OMP $N $T pd results_UTWMPI_OMP_dynamic_4.csv $nodes
            done
        done
    done
}
